...
data_train0 = np.load('dens.npy')[0:2000]
...
i = 10
img = data_train[i,:]
plt.imshow(img.reshape((10, 10)), cmap='Greys_r')
print("Label: {}".format(1))
...
def get_generator(noise_img, output_dim, is_train=True, alpha=0.2):
    ...
    with tf.variable_scope("generator", reuse=(not is_train)):
        layer1 = tf.layers.dense(noise_img, 5*5*4, activation=tf.nn.relu)          #(N,10)-->(N,5*5*4)
        layer1 = tf.reshape(layer1, [-1, 5, 5, 4])          #(N,5*5*4)-->(N,5,5,4)
        # batch normalization
        layer1 = tf.layers.batch_normalization(layer1, training=is_train)
        # dropout
        layer1 = tf.nn.dropout(layer1, keep_prob=0.8)                  
        
        layer2 = tf.layers.conv2d_transpose(layer1, 8, 4, strides=2, padding='same',activation=tf.nn.relu)     #(N,5,5,4)--> (N,10,10,8)
        layer2 = tf.layers.batch_normalization(layer2, training=is_train)
        layer2 = tf.nn.dropout(layer2, keep_prob=0.8)
        
        layer3 = tf.layers.conv2d_transpose(layer2, 16, 4, strides=1, padding='same',activation=tf.nn.relu)         #(N,10,10,8)--> (N,10,10,16)
        layer3 = tf.layers.batch_normalization(layer3, training=is_train)
        layer3 = tf.nn.dropout(layer3, keep_prob=0.8)
        
        logits0 = tf.layers.conv2d_transpose(layer3, output_dim, 5, strides=1, padding='same')     #(N,10,10,16)--> (N,10,10,1)
        temp=tf.reshape(logits0,[tf.shape(logits0)[0],nelx*nelx])
        temp1=tf.contrib.framework.sort(temp,axis=1)
        temp2=(temp1[:,79]+temp1[:,80])/2#
...
def get_discriminator(inputs_img, reuse=False, alpha=0.2):
    with tf.variable_scope("discriminator", reuse=reuse):
        layer1 = tf.layers.conv2d(inputs_img, 4, 2, strides=1, padding='same',activation=tf.nn.relu) #(N,10,10,1)--> (N,10,10,4)
        layer1 = tf.layers.batch_normalization(layer1, training=True)
        layer1 = tf.nn.dropout(layer1, keep_prob=0.8)
        
        layer2 = tf.layers.conv2d(layer1,8, 3, strides=2, padding='same',activation=tf.nn.relu)   #(N,10,10,4)--> (N,5,5,8)
        layer2 = tf.layers.batch_normalization(layer2, training=True)
        layer2 = tf.nn.dropout(layer2, keep_prob=0.8)
        
        layer3 = tf.layers.conv2d(layer2, 16, 3, strides=2, padding='same',activation=tf.nn.relu)   #(N,5,5,8)--> (N,3,3,16)
        layer3 = tf.layers.batch_normalization(layer3, training=True)
        layer3 = tf.nn.dropout(layer3, keep_prob=0.8)
        
        flatten = tf.reshape(layer3, (-1, 3*3*16))
...
def plot_images(samples,steps):
    ...
    for img, ax in zip(samples, axes):
        ax.imshow(img.reshape((10, 10)), cmap='Greys_r')
    ...
    with tf.Session() as sess:
        ...
        for e in range(epochs):
            ...
            for batch_i in range(data_train.shape[0]//batch_size):
                ...
                if steps % 1000 == 0: #and e>=epochs/2:
                    ...
                    #np.savetxt('samp.csv',samples.reshape(samples.shape[0],100),delimiter=',')

                    print('heat source number=',np.sum(1.0*(samples[0:4,:,:].reshape(4,100)>=0.5),axis=1))
                    print("Epoch {}/{}....".format(e+1, epochs), 
                          "Discriminator Loss: {:.4f}....".format(train_loss_d),
                          "Generator Loss: {:.4f}....". format(train_loss_g))

with tf.Graph().as_default():
    train(noise_size, [-1, 10, 10, 1], batch_size, n_samples)
